...
const disconnect=async(r:Request,e:Env,playerId:number)=>{
  ...
  const linked=await e.DB.prepare(`SELECT a.id,a.display_name,a.is_owner,i.provider_username FROM accounts a LEFT JOIN account_identities i ON i.account_id=a.id AND i.provider='discord' WHERE a.player_id=? LIMIT 1`).bind(playerId).first<Linked>();
  if(!linked)return new Response("No linked account",{status:400});if(linked.is_owner)return redirect(`/players/${playerId}?disconnecterror=owner`,r);
...
export default {async fetch(request:Request,env:Env):Promise<Response>{const u=new URL(request.url),m=u.pathname.match(/^\/players\/(\d+)\/disconnect-discord$/);if(request.method==="POST"&&m)return disconnect(request,env,Number(m[1]));return (app as any).fetch(request,env)}} satisfies ExportedHandler<Env>;
