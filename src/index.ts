...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Alliance Platform Setup</title>
<style>:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui,sans-serif}body{margin:0;min-height:100vh;display:grid;place-items:center;background:#0c1220;color:#eef3ff}main{width:min(92vw,520px);background:#151d2e;border:1px solid #2b3850;border-radius:18px;padding:28px;box-shadow:0 24px 70px rgba(0,0,0,.35)}h1{margin:0 0 8px;font-size:1.7rem}p{color:#b8c4d9;line-height:1.55}label{display:block;margin:22px 0 8px;font-weight:700}input{width:100%;box-sizing:border-box;padding:13px 14px;border-radius:10px;border:1px solid #3a4964;background:#0e1626;color:white;font-size:1rem}button{width:100%;margin-top:14px;padding:13px 16px;border:0;border-radius:10px;background:#5865f2;color:white;font-size:1rem;font-weight:800;cursor:pointer}.notice{margin:14px 0 0;padding:12px;border-radius:10px;background:#24191c;color:#ffd5dc}.small{font-size:.9rem;color:#90a0bb}</style></head>
<body><main><h1>Create the first owner</h1><p>Enter the one-time setup key you stored in Cloudflare. You will then be sent to Discord to verify your identity.</p>${message ? `<div class="notice">${escapeHtml(message)}</div>` : ""}<form method="post" action="/setup/discord/start" autocomplete="off"><label for="setup_key">Setup key</label><input id="setup_key" name="setup_key" type="password" required autocomplete="off"><button type="submit">Verify with Discord</button></form><p class="small">The setup key is never stored in the database or written to application logs.</p></main></body></html>`;

const setupDonePage = (name: string): string => `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Owner Connected</title>
<style>:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui,sans-serif}body{margin:0;min-height:100vh;display:grid;place-items:center;background:#0c1220;color:#eef3ff}main{width:min(92vw,520px);background:#151d2e;border:1px solid #2b3850;border-radius:18px;padding:28px}h1{margin-top:0}p{color:#b8c4d9;line-height:1.55}strong{color:white}</style></head>
<body><main><h1>Discord owner verified</h1><p><strong>${escapeHtml(name)}</strong> is now the installation owner.</p><p>The owner-creation route is locked. The next stage is configuring the alliance and Discord server, then enabling normal player sign-up and approval.</p></main></body></html>`;
...
const clearSetupCookie = (): string => `${SETUP_COOKIE}=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0`;
...
const exchangeDiscordCode = async (code: string, env: Env): Promise<DiscordTokenResponse> => {
  const body = new URLSearchParams({ client_id: env.DISCORD_CLIENT_ID, client_secret: env.DISCORD_CLIENT_SECRET, grant_type: "authorization_code", code, redirect_uri: `${env.APP_URL}/auth/discord/callback` });
...
const finishDiscordSetup = async (request: Request, env: Env): Promise<Response> => {
  if (await ownerExists(env)) return html(setupPage("An owner account already exists. First-owner setup is locked."), { status: 409, headers: { "set-cookie": clearSetupCookie() } });
  const url = new URL(request.url);
  const code = url.searchParams.get("code") ?? "";
  const state = url.searchParams.get("state") ?? "";
  if (!code || !state || !(await validateSetupState(request, state, env.SETUP_KEY))) return html(setupPage("Discord verification could not be validated. Please start again."), { status: 400, headers: { "set-cookie": clearSetupCookie() } });
  ...
  try {
    ...
    if (existingIdentity) return html(setupPage("That Discord account is already linked to this installation."), { status: 409, headers: { "set-cookie": clearSetupCookie() } });
    ...
    return html(setupDonePage(displayName), { headers: { "set-cookie": clearSetupCookie() } });
  } catch (error) {
    console.error("Discord owner bootstrap failed", error);
    return html(setupPage("Discord verification failed. Please try again."), { status: 502, headers: { "set-cookie": clearSetupCookie() } });
  }
};
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    const url = new URL(request.url);
    if (request.method === "GET" && url.pathname === "/") return json({ name: "Alliance Platform", status: "ok", version: "0.1.0" });
    ...
    if (request.method === "GET" && url.pathname === "/auth/discord/callback") return finishDiscordSetup(request, env);
