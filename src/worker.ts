...
const ASSET_SLOTS:Record<string,{setting:string;key:string;types:string[]}> = {
...
const deleteSetting = async(e:Env,key:string) => e.DB.prepare("DELETE FROM settings WHERE key=?").bind(key).run();

const cleanupLegacyAdminLogo = async(e:Env,a:AccountRow) => {
  const legacy=await e.DB.prepare("SELECT value FROM settings WHERE key='brand_admin_logo'").first<{value:string}>();
  if(!legacy?.value)return;
  await e.ASSETS.delete(legacy.value);
  await deleteSetting(e,"brand_admin_logo");
  await audit(e,a,"branding.asset.removed","admin",{slot:"admin",key:legacy.value,reason:"legacy compact admin logo removed"});
};

const uploadBranding = async(r:Request,e:Env,a:AccountRow) => {
  const f=await r.formData(),slot=String(f.get("slot")??"");
  await cleanupLegacyAdminLogo(e,a);
  if(slot==="text"){
    const vals=[
  ...
  if(slot==="remove"){
    const assetSlot=String(f.get("asset_slot")??"");
    const cfg=ASSET_SLOTS[assetSlot];
    if(!cfg)return Response.redirect(new URL("/settings/alliance/branding?error=file",r.url),302);
    const existing=await e.DB.prepare("SELECT value FROM settings WHERE key=?").bind(cfg.setting).first<{value:string}>();
    if(existing?.value){
      await e.ASSETS.delete(existing.value);
      await deleteSetting(e,cfg.setting);
      await audit(e,a,"branding.asset.removed",assetSlot,{slot:assetSlot,key:existing.value});
    }
    return Response.redirect(new URL(`/settings/alliance/branding?saved=removed-${encodeURIComponent(assetSlot)}`,r.url),302);
  }
  const cfg=ASSET_SLOTS[slot],file=f.get("asset");
  ...
  if(existing?.value&&existing.value!==cfg.key)await e.ASSETS.delete(existing.value);
  await audit(e,a,"branding.asset.updated",slot,{slot,key:cfg.key,contentType:file.type,size:file.size,replacedKey:existing?.value||null});
...
const decorate = async(res:Response,e:Env) => {
  ...
  const s=await getSettings(e),logo=s.brand_main_logo,css=shellStyles+themeCss(s);
  ...
  async fetch(request:Request,env:Env):Promise<Response>{
    ...
    if(u.pathname==="/settings/alliance/branding"){
      ...
      if(request.method==="GET"){
        ...
        const message=error?(error==="theme"?"That theme could not be saved. Please choose valid colours.":"That image could not be uploaded. Use PNG, JPG or WebP (app/browser icon: PNG/ICO), maximum 5 MB."):saved?(saved==="reset"?"Theme reset to the default Midnight colours.":saved.startsWith("removed-")?"Image removed and deleted from storage.":"Branding saved successfully."):undefined;
