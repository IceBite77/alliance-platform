import runtime from "./leadership_audit";
...
const SETTINGS_CHILDREN=["settings.details","settings.branding","settings.discord","settings.ranks","settings.players"];
const settingsPermission=(path:string)=>{
  if(path==="/leadership/settings/details")return "settings.details";
  if(path==="/leadership/settings/branding")return "settings.branding";
...
// Temporary compatibility adapter for the old Settings renderer. Authorization is
// performed before this adapter is used. It can be deleted once Settings is moved
// onto the shared Leadership handler instead of the legacy owner-only renderer.
const legacySettingsEnv=(env:Env)=>{
  const db=new Proxy(env.DB as any,{get(target,prop,receiver){if(prop==="prepare")return(sql:string)=>target.prepare(String(sql).replaceAll(" AND a.is_owner=1 LIMIT 1"," LIMIT 1").replaceAll(" AND a.is_owner = 1 LIMIT 1"," LIMIT 1").replaceAll(" AND a.is_owner=1",""));const value=Reflect.get(target,prop,receiver);return typeof value==="function"?value.bind(target):value;}});
  return new Proxy(env as any,{get(target,prop,receiver){if(prop==="DB")return db;return Reflect.get(target,prop,receiver)}});
};
const canOpenSettings=async(env:Env,actor:any,path:string)=>{
  if(await playerPermitted(env,actor,"settings.manage"))return true;
  const child=settingsPermission(path);
  if(child)return playerPermitted(env,actor,child);
  if(path==="/leadership/settings"){
    for(const key of SETTINGS_CHILDREN)if(await playerPermitted(env,actor,key))return true;
  }
  return false;
};
const hideUnavailableSettingsCards=async(env:Env,actor:any,html:string)=>{
  const routes:[string,string][]=[
    ["/leadership/settings/details","settings.details"],["/leadership/settings/branding","settings.branding"],
    ["/leadership/settings/discord","settings.discord"],["/leadership/settings/ranks","settings.ranks"],
    ["/leadership/settings/players","settings.players"]
  ];
  if(await playerPermitted(env,actor,"settings.manage"))return html;
  for(const [route,key] of routes){
    if(await playerPermitted(env,actor,key))continue;
    const escaped=route.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");
    html=html.replace(new RegExp(`<a\\b[^>]*href=["']${escaped}["'][^>]*>[\\s\\S]*?<\\/a>`,"g"),"");
  }
  return html;
};
...
export default {async fetch(request:Request,env:Env,ctx:ExecutionContext):Promise<Response>{
  ...
  if(request.method==="POST"&&accountAction){const rewritten=new URL(request.url);rewritten.pathname=`/players/${accountAction[1]}/${accountAction[2]}`;const response=await(accountLifecycle as any).fetch(new Request(rewritten.toString(),request),env,ctx);const location=response.headers.get("location");if(location){const target=new URL(location);if(target.pathname.startsWith("/players")){target.pathname=target.pathname.replace(/^\/players/,"/leadership/players")||"/leadership/players";const headers=new Headers(response.headers);headers.set("location",target.toString());return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}}return response;}

  if(url.pathname==="/leadership/players"||url.pathname.startsWith("/leadership/players/")){const previewUrl=new URL(request.url);previewUrl.pathname=url.pathname.replace(/^\/leadership\/players/,"/leadership/players-preview")||"/leadership/players-preview";const response=await(playersPreview as any).fetch(new Request(previewUrl.toString(),request),env,ctx);const location=response.headers.get("location");if(location){const target=new URL(location);if(target.pathname.startsWith("/leadership/players-preview")){target.pathname=target.pathname.replace(/^\/leadership\/players-preview/,"/leadership/players")||"/leadership/players";const headers=new Headers(response.headers);headers.set("location",target.toString());return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}}if(response.headers.get("content-type")?.includes("text/html")){const body=(await response.text()).replaceAll("/leadership/players-preview","/leadership/players");return new Response(body,{status:response.status,statusText:response.statusText,headers:response.headers});}return response;}
  ...
  if(request.method==="GET"&&url.pathname==="/leadership/security"){const rewritten=new URL(request.url);rewritten.pathname="/security/access";return(securityDashboard as any).fetch(new Request(rewritten.toString(),request),env,ctx);}
  if(request.method==="POST"&&url.pathname.startsWith("/leadership/security/")){const rewritten=new URL(request.url);rewritten.pathname=url.pathname.replace(/^\/leadership\/security/,"/security/access");return(securityDashboard as any).fetch(new Request(rewritten.toString(),request),env,ctx);}
  ...
  if(internalSettings){
    ...
    if(!await canOpenSettings(env,actor,url.pathname))return new Response("Forbidden",{status:403});
    const rewritten=new URL(request.url);rewritten.pathname=internalSettings;
    const response=await(runtime as any).fetch(new Request(rewritten.toString(),request),legacySettingsEnv(env),ctx);
    ...
    if(response.headers.get("content-type")?.includes("text/html")){let body=rewriteSettingsHtml(await response.text());if(url.pathname==="/leadership/settings")body=await hideUnavailableSettingsCards(env,actor,body);return new Response(body,{status:response.status,statusText:response.statusText,headers:response.headers});}
    return response;
  }
