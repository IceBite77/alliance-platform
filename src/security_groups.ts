...
export default {async fetch(request:Request,env:Env,ctx:ExecutionContext):Promise<Response>{
  ...
  for(const group of groups){const heading=`<h3>${esc(group.name)}</h3>`;const linked=`<h3>${esc(group.name)}</h3><a class="groupclick" href="/leadership/security/groups/${group.id}" aria-label="Manage ${esc(group.name)}"></a>`;body=body.replace(heading,linked)}
  body=body.replace('</style>',`.group{position:relative;transition:border-color .15s ease,background .15s ease,transform .15s ease}.group:hover{border-color:#526b94;background:#121d30;transform:translateY(-1px)}.groupclick{position:absolute;inset:0;z-index:2;border-radius:13px}.group:has(.groupclick){cursor:pointer}.group:has(.groupclick) h3{color:#eef3ff}.group:has(.groupclick)::after{content:'›';position:absolute;right:14px;bottom:12px;color:#6f82a2;font-size:1.2rem;font-weight:900}.group:has(.groupclick) .stats{margin-right:18px}@media(max-width:650px){.group:hover{transform:none}}</style>`);
