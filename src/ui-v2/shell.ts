...
function navigation(context:UiV2Context,activePath:string|undefined){
  const sections=["Navigation","Management","Security","Settings"] as const;
...
.confirm-backdrop[hidden]{display:none}.confirm-backdrop{position:fixed;inset:0;z-index:1000;display:grid;place-items:center;padding:20px;background:rgba(4,8,15,.74);backdrop-filter:blur(5px)}.confirm-dialog{width:min(100%,460px);padding:24px;border:1px solid var(--ui-line-strong);border-radius:16px;background:var(--ui-surface);box-shadow:0 28px 75px rgba(0,0,0,.55)}.confirm-dialog .eyebrow{margin-bottom:7px}.confirm-dialog h2{margin:0;font-size:1.3rem}.confirm-message{margin:12px 0 0;color:var(--ui-soft);font-size:.9rem;line-height:1.6}.confirm-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:22px}.confirm-button{display:inline-flex;align-items:center;justify-content:center;min-height:42px;padding:10px 16px;border:1px solid var(--ui-line-strong);border-radius:9px;background:var(--ui-surface-3);color:var(--ui-navigation);font-size:.78rem;font-weight:900;cursor:pointer;transition:border-color .15s ease,color .15s ease,background .15s ease,box-shadow .15s ease}.confirm-button:hover{border-color:var(--ui-hover);color:var(--ui-hover);box-shadow:0 0 16px color-mix(in srgb,var(--ui-hover) 10%,transparent)}.confirm-button.primary{border-color:color-mix(in srgb,var(--ui-danger) 55%,var(--ui-line));background:color-mix(in srgb,var(--ui-danger) 12%,var(--ui-surface-3));color:var(--ui-danger)}.confirm-button.primary:hover{border-color:var(--ui-danger);background:color-mix(in srgb,var(--ui-danger) 20%,var(--ui-surface-3));color:var(--ui-text)}body.confirm-open{overflow:hidden}
...
const script=`<script>(()=>{const mobileButton=document.querySelector('[data-menu-button]'),mobileMenu=document.querySelector('[data-mobile-menu]'),desktopButton=document.querySelector('[data-leadership-menu-button]'),desktopMenu=document.querySelector('[data-leadership-menu]');mobileButton?.addEventListener('click',()=>{const open=mobileMenu.classList.toggle('open');mobileButton.setAttribute('aria-expanded',String(open));});desktopButton?.addEventListener('click',event=>{event.stopPropagation();const open=desktopMenu.classList.toggle('open');desktopButton.setAttribute('aria-expanded',String(open));});desktopMenu?.addEventListener('click',event=>event.stopPropagation());document.addEventListener('click',()=>{desktopMenu?.classList.remove('open');desktopButton?.setAttribute('aria-expanded','false')});const modal=document.querySelector('[data-confirm-modal]'),modalTitle=modal?.querySelector('[data-confirm-title]'),modalMessage=modal?.querySelector('[data-confirm-message]'),confirmButton=modal?.querySelector('[data-confirm-accept]'),cancelButton=modal?.querySelector('[data-confirm-cancel]');let pending=null,lastFocus=null;const closeConfirm=()=>{modal.hidden=true;document.body.classList.remove('confirm-open');pending=null;if(lastFocus?.focus)lastFocus.focus()};document.addEventListener('submit',event=>{const form=event.target;if(!(form instanceof HTMLFormElement))return;const submitter=event.submitter instanceof HTMLElement?event.submitter:null,source=submitter?.dataset.confirm?submitter:form.dataset.confirm?form:null;if(!source)return;if(form.dataset.confirmed==='true'){delete form.dataset.confirmed;return}event.preventDefault();lastFocus=document.activeElement;pending={form,submitter:event.submitter};modalTitle.textContent=source.dataset.confirmTitle||'Please confirm';modalMessage.textContent=source.dataset.confirm;confirmButton.textContent=source.dataset.confirmButton||'Confirm';modal.hidden=false;document.body.classList.add('confirm-open');cancelButton.focus()});cancelButton?.addEventListener('click',closeConfirm);modal?.addEventListener('click',event=>{if(event.target===modal)closeConfirm()});confirmButton?.addEventListener('click',()=>{if(!pending)return;const action=pending;modal.hidden=true;document.body.classList.remove('confirm-open');pending=null;action.form.dataset.confirmed='true';action.form.requestSubmit(action.submitter||undefined)});document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!modal?.hidden){closeConfirm();return}if(event.key==='Escape'){desktopMenu?.classList.remove('open');desktopButton?.setAttribute('aria-expanded','false');mobileMenu?.classList.remove('open');mobileButton?.setAttribute('aria-expanded','false')}});})();</script>`;
