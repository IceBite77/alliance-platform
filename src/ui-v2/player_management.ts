...
type SquadLoadout={squad_number:number;power:number|null;drone_level:number|null;drone_power:number|null};
type OverlordLoadout={level:number|null;power:number|null;assigned_squad:number|null};
...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  const [player,ranks,maxBase,maxOverlord,linked,history,performance,squadLoadouts,overlord,snapshots]=await Promise.all([
    ...
    env.DB.prepare("SELECT s.squad_number,s.power,d.level drone_level,d.power drone_power FROM player_squads s LEFT JOIN player_squad_drones d ON d.player_id=s.player_id AND d.squad_number=s.squad_number WHERE s.player_id=? UNION SELECT d.squad_number,NULL power,d.level drone_level,d.power drone_power FROM player_squad_drones d WHERE d.player_id=? AND NOT EXISTS (SELECT 1 FROM player_squads s WHERE s.player_id=d.player_id AND s.squad_number=d.squad_number) ORDER BY squad_number").bind(playerId,playerId).all<SquadLoadout>(),
    env.DB.prepare("SELECT level,power,assigned_squad FROM player_companions WHERE player_id=? AND companion_type='overlord' LIMIT 1").bind(playerId).first<OverlordLoadout>(),
  ...
  const current=performance??{total_strength:null,total_hero_power:null},squadRows=squadLoadouts.results??[],squads=Array.from({length:4},(_,index)=>squadRows.find(row=>row.squad_number===index+1)??{squad_number:index+1,power:null,drone_level:null,drone_power:null}),currentOverlord=overlord??{level:null,power:null,assigned_squad:null},snapshotRows=snapshots.results??[];
  ...
  const squadEditors=squads.map(squad=>`<section class="loadout-card"><h4>Squad ${squad.squad_number}</h4><div class="loadout-fields"><label class="field"><span>Squad power</span><input name="squad_${squad.squad_number}_power" inputmode="decimal" value="${squad.power??""}" placeholder="Not set"></label><label class="field"><span>Drone power</span><input name="drone_${squad.squad_number}_power" inputmode="decimal" value="${squad.drone_power??""}" placeholder="Not set"></label><label class="field"><span>Tactical Drone level</span><input name="drone_${squad.squad_number}_level" type="number" inputmode="numeric" min="1" max="999" value="${squad.drone_level??""}" placeholder="Not set"></label></div></section>`).join("");
  ...
  const performanceEditor=context.user.canEditPlayers?`<form class="performance-editor" method="post" action="/ui-v2/players/${player.id}/performance"><h3>Update performance and squad setup</h3><p>Add figures as they become available. Power accepts commas or abbreviations such as 120M; blank fields clear that item.</p><div class="performance-totals"><label class="field"><span>Total Strength</span><input name="total_strength" inputmode="decimal" value="${current.total_strength??""}" placeholder="e.g. 120M"></label><label class="field"><span>Total Hero Power</span><input name="total_hero_power" inputmode="decimal" value="${current.total_hero_power??""}" placeholder="Add later"></label></div><div class="loadout-grid">${squadEditors}</div><section class="loadout-card overlord-card"><h4>Overlord</h4><div class="overlord-fields"><label class="field"><span>Overlord power</span><input name="overlord_power" inputmode="decimal" value="${currentOverlord.power??""}" placeholder="Not set"></label><label class="field"><span>Overlord level</span><input name="overlord_level" type="number" inputmode="numeric" min="1" max="${maxOverlord}" value="${currentOverlord.level??""}" placeholder="Not set"><small>Current platform maximum: ${maxOverlord}</small></label><label class="field"><span>Used with</span><select name="overlord_squad"><option value="">Not assigned</option>${squadOptions}</select></label></div></section><div class="form-actions"><button class="manage-button" type="submit">Save performance</button></div></form>`:"";
  const overlordSummary=`<section class="loadout-card overlord-card"><h4>Overlord</h4><p class="membership-note"><strong>${currentOverlord.level?`Level ${currentOverlord.level}`:"Level not set"}</strong> · ${currentOverlord.assigned_squad?`Used with Squad ${currentOverlord.assigned_squad}`:"Squad not assigned"} · Power ${esc(compactPower(currentOverlord.power))}</p></section>`;
  const performanceCard=`<section class="manage-card"><h2>Roster Performance</h2><p>Four squad slots each retain their own power and Tactical Drone. The Overlord can be assigned to whichever squad the player uses it with.</p><div class="performance-grid">${stat("Total Strength",current.total_strength)}${squads.map(squad=>stat(`Squad ${squad.squad_number} Power`,squad.power)).join("")}${stat("Total Hero Power",current.total_hero_power)}</div>${overlordSummary}${performanceEditor}${snapshotMarkup?`<div class="snapshot-wrap"><table class="snapshot-table"><thead><tr><th>Performance update</th><th>Total Strength</th><th>Squad 1</th><th>Squad 2</th><th>Squad 3</th><th>Squad 4</th><th>Hero Power</th></tr></thead><tbody>${snapshotMarkup}</tbody></table></div>`:'<div class="profile-empty">No performance history has been recorded yet.</div>'}</section>`;
  ...
  const body=`${managementCss}${profileCss}${loadoutCss}<a class="manage-button secondary manage-return" href="/ui-v2/players">← Players</a>${notice}<div class="profile-layout"><div class="profile-stack">${details}${performanceCard}${historyCard}${notesCard}</div><aside class="profile-stack">${account}${membership}</aside></div>`;
...
async function updatePerformance(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  const squads=Array.from({length:4},(_,index)=>{const number=index+1;return{number,power:parsePerformancePower(form.get(`squad_${number}_power`)),dronePower:parsePerformancePower(form.get(`drone_${number}_power`)),droneLevel:parseOptionalLevel(form.get(`drone_${number}_level`),999)}});
  const overlordPower=parsePerformancePower(form.get("overlord_power")),overlordLevel=parseOptionalLevel(form.get("overlord_level"),maxOverlord),overlordSquadRaw=String(form.get("overlord_squad")??"").trim(),overlordSquad=overlordSquadRaw?Number(overlordSquadRaw):null;
  if(!strength.valid||!heroes.valid||!overlordPower.valid||!overlordLevel.valid||overlordSquad!==null&&(!Number.isInteger(overlordSquad)||overlordSquad<1||overlordSquad>4)||squads.some(squad=>!squad.power.valid||!squad.dronePower.valid||!squad.droneLevel.valid))return redirect(request,`/ui-v2/players/${playerId}?error=performance`);
  const [player,old,oldSquads,oldOverlord]=await Promise.all([
    loadStoredPlayer(env,playerId),
    env.DB.prepare("SELECT player_power total_strength,total_hero_power FROM players WHERE id=?").bind(playerId).first<PlayerPerformance>(),
    env.DB.prepare("SELECT s.squad_number,s.power,d.level drone_level,d.power drone_power FROM player_squads s LEFT JOIN player_squad_drones d ON d.player_id=s.player_id AND d.squad_number=s.squad_number WHERE s.player_id=? UNION SELECT d.squad_number,NULL power,d.level drone_level,d.power drone_power FROM player_squad_drones d WHERE d.player_id=? AND NOT EXISTS (SELECT 1 FROM player_squads s WHERE s.player_id=d.player_id AND s.squad_number=d.squad_number) ORDER BY squad_number").bind(playerId,playerId).all<SquadLoadout>(),
    env.DB.prepare("SELECT level,power,assigned_squad FROM player_companions WHERE player_id=? AND companion_type='overlord' LIMIT 1").bind(playerId).first<OverlordLoadout>()
  ...
  const next={total_strength:strength.value,total_hero_power:heroes.value,squads:squads.map(squad=>({squad_number:squad.number,power:squad.power.value,tactical_drone:{level:squad.droneLevel.value,power:squad.dronePower.value}})),overlord:{level:overlordLevel.value,power:overlordPower.value,assigned_squad:overlordSquad}};
  ...
  for(const squad of squads){
    ...
    statements.push(squad.droneLevel.value===null&&squad.dronePower.value===null?env.DB.prepare("DELETE FROM player_squad_drones WHERE player_id=? AND squad_number=?").bind(playerId,squad.number):env.DB.prepare("INSERT INTO player_squad_drones (player_id,squad_number,level,power) VALUES (?,?,?,?) ON CONFLICT(player_id,squad_number) DO UPDATE SET level=excluded.level,power=excluded.power,updated_at=CURRENT_TIMESTAMP").bind(playerId,squad.number,squad.droneLevel.value,squad.dronePower.value));
  }
  statements.push(overlordLevel.value===null&&overlordPower.value===null&&overlordSquad===null?env.DB.prepare("DELETE FROM player_companions WHERE player_id=? AND companion_type='overlord'").bind(playerId):env.DB.prepare("INSERT INTO player_companions (player_id,companion_type,level,power,assigned_squad) VALUES (?,'overlord',?,?,?) ON CONFLICT(player_id,companion_type) DO UPDATE SET level=excluded.level,power=excluded.power,assigned_squad=excluded.assigned_squad,updated_at=CURRENT_TIMESTAMP").bind(playerId,overlordLevel.value,overlordPower.value,overlordSquad));
  ...
  statements.push(auditStatement(env,context,"player.performance_updated","player",String(playerId),{...old,squads:oldSquads.results??[],overlord:oldOverlord??null},next,{update_source:"manual"}));
  await env.DB.batch(statements);
