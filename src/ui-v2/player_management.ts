...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  const current=performance??{total_strength:null,total_hero_power:null},squadRows=squadLoadouts.results??[],squads=Array.from({length:4},(_,index)=>squadRows.find(row=>row.squad_number===index+1)??{squad_number:index+1,power:null}),droneLevel=tacticalDrone?.level??null,currentOverlord=overlord??{level:null,power:null,assigned_squad:null},snapshotRows=snapshots.results??[];
  ...
  const squadEditors=squads.map(squad=>`<section class="loadout-card"><h4>Squad ${squad.squad_number}</h4><label class="field"><span>Squad power</span><input name="squad_${squad.squad_number}_power" inputmode="decimal" value="${squad.power??""}" placeholder="Not set"></label></section>`).join("");
  ...
  const performanceCard=`<section class="manage-card"><h2>Roster Performance</h2><p>Each squad retains its own power. The Tactical Drone is shared across all squads, while the Overlord can be assigned to a specific squad.</p><div class="performance-grid">${stat("Total Strength",current.total_strength)}${squads.map(squad=>stat(`Squad ${squad.squad_number} Power`,squad.power)).join("")}${stat("Total Hero Power",current.total_hero_power)}${levelStat("Tactical Drone",droneLevel)}</div>${overlordSummary}${performanceEditor}${snapshotMarkup?`<div class="snapshot-wrap"><table class="snapshot-table"><thead><tr><th>Performance update</th><th>Total Strength</th><th>Squad 1</th><th>Squad 2</th><th>Squad 3</th><th>Squad 4</th><th>Hero Power</th></tr></thead><tbody>${snapshotMarkup}</tbody></table></div>`:'<div class="profile-empty">No performance history has been recorded yet.</div>'}</section>`;
...
async function updatePerformance(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  if(!strength.valid||!heroes.valid||!droneLevel.valid||!overlordPower.valid||!overlordLevel.valid||overlordSquad!==null&&(!Number.isInteger(overlordSquad)||overlordSquad<1||overlordSquad>4)||squads.some(squad=>!squad.power.valid))return redirect(request,`/ui-v2/players/${playerId}?error=performance`);
  ...
  const next={total_strength:strength.value,total_hero_power:heroes.value,squads:squads.map(squad=>({squad_number:squad.number,power:squad.power.value})),tactical_drone:{level:droneLevel.value},overlord:{level:overlordLevel.value,power:overlordPower.value,assigned_squad:overlordSquad}};
