...
type RosterSnapshot=PlayerPerformance&{recorded_at:string};
...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  const [player,ranks,maxBase,linked,history,performance,snapshots]=await Promise.all([
    ...
    env.DB.prepare("SELECT total_strength,squad_1_power,total_hero_power,recorded_at FROM player_roster_snapshots WHERE player_id=? ORDER BY recorded_at DESC,id DESC LIMIT 8").bind(playerId).all<RosterSnapshot>()
  ]);
  ...
  const snapshotMarkup=snapshotRows.map(row=>`<tr><td>${esc(historyTime(row.recorded_at))}</td><td>${esc(compactPower(row.total_strength))}</td><td>${esc(compactPower(row.squad_1_power))}</td><td>${esc(compactPower(row.total_hero_power))}</td></tr>`).join("");
  const performanceCard=`<section class="manage-card"><h2>Roster Performance</h2><p>Current figures come from the first roster upload and are refreshed by each weekly spreadsheet.</p><div class="performance-grid">${stat("Total Strength",current.total_strength)}${stat("Squad 1 Power",current.squad_1_power)}${stat("Total Hero Power",current.total_hero_power)}</div>${snapshotMarkup?`<div class="snapshot-wrap"><table class="snapshot-table"><thead><tr><th>Roster update</th><th>Total Strength</th><th>Squad 1</th><th>Hero Power</th></tr></thead><tbody>${snapshotMarkup}</tbody></table></div>`:'<div class="profile-empty">No roster upload history has been recorded yet.</div>'}</section>`;
...
export async function handleUiV2PlayerManagement(request:Request,env:UiV2Env,context:UiV2Context):Promise<Response|null>{
  ...
  const profile=path.match(/^\/ui-v2\/players\/(\d+)(?:\/(update|deactivate|reactivate|disconnect|login-enable|login-disable|notes))?$/);
