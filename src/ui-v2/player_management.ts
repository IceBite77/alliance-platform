...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  if(linked&&!linked.is_owner&&context.user.canManageMembership)accountControls.push(`<form method="post" action="/ui-v2/players/${player.id}/disconnect" data-confirm="Disconnect this Discord login? Their sessions and access groups will be removed, but the player record will be kept." data-confirm-title="Disconnect Discord" data-confirm-button="Disconnect" onsubmit="return confirm('Disconnect this Discord login? Their sessions and access groups will be removed, but the player record will be kept.');"><button class="manage-button danger" type="submit">Disconnect Discord</button></form>`);
