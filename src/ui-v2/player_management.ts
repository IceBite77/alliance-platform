...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  const body=`${managementCss}${profileCss}${loadoutCss}<a class="manage-button secondary manage-return" href="/ui-v2/players">← Players</a>${notice}<div class="profile-layout"><div class="profile-stack">${details}${performanceCard}${notesCard}</div><aside class="profile-stack">${account}${historyCard}${membership}</aside></div>`;
