...
async function accessPage(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  const [pending,players]=await Promise.all([
    env.DB.prepare("SELECT a.id,a.display_name,a.claimed_player_name,i.provider_username FROM accounts a LEFT JOIN account_identities i ON i.account_id=a.id AND i.provider='discord' WHERE a.approval_status='pending' AND a.is_active=1 AND a.is_owner=0 ORDER BY a.created_at").all<PendingAccount>(),
  ...
  const notice=saved?`<div class="manage-notice">${saved==="approved"?"Discord access approved and linked to the player.":saved==="blocked"?"Discord account blocked and removed from the approval queue.":"Access request rejected."}</div>`:error?`<div class="manage-notice error">${error==="linked"?"That player is already linked to another account.":"Choose an available active player and try again."}</div>`:"";
  const cards=rows.map(account=>{const claimed=account.claimed_player_name?.trim()||"",discord=account.provider_username?`@${account.provider_username}`:account.display_name,options=available.map(player=>`<option value="${player.id}"${claimed&&player.display_name.localeCompare(claimed,undefined,{sensitivity:"accent"})===0?" selected":""}>${esc(playerLabel(player))}</option>`).join("");return `<article class="access-card"><div class="access-person"><strong>${esc(claimed||discord)}</strong><span>${claimed?`Claimed player name · Discord: ${esc(discord)}`:"Player name not supplied · Discord sign-in waiting"}</span></div><form class="access-approve" method="post" action="/ui-v2/players/access/${account.id}/approve"><label class="field"><span>Match to player</span><select name="player_id" required><option value="">Choose player…</option>${options}</select></label><button class="manage-button" type="submit">Approve</button></form><div class="access-decisions"><form method="post" action="/ui-v2/players/access/${account.id}/reject" data-confirm="Reject this request? The Discord account may request access again later." data-confirm-title="Reject access request" data-confirm-button="Reject request"><button class="manage-button secondary" type="submit">Reject</button></form><form method="post" action="/ui-v2/players/access/${account.id}/block" data-confirm="Block this Discord account? It will not be allowed back into the approval queue until it is unblocked in Security & Access." data-confirm-title="Block Discord account" data-confirm-button="Block account"><button class="manage-button danger" type="submit">Block</button></form></div></article>`}).join("");
  const body=`${managementCss}${notice}<div class="access-summary"><div class="access-summary-count"><strong>${rows.length}</strong><span>${rows.length===1?"account is":"accounts are"} waiting for approval</span></div><span>${available.length} unlinked active ${available.length===1?"player":"players"} available</span></div><div class="access-list">${cards||'<div class="empty-access">No Discord accounts are waiting for access.</div>'}</div>`;
...
async function blockAccount(request:Request,env:UiV2Env,context:UiV2Context,accountId:number){
  if(!context.user.canApproveAccounts||!playerSameOrigin(request,env))return new Response("Forbidden",{status:403});
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("INSERT INTO account_security_blocks (account_id,reason,blocked_at,blocked_by_account_id,unblocked_at,unblocked_by_account_id) VALUES (?,'Repeated or unauthorised access attempts',CURRENT_TIMESTAMP,?,NULL,NULL) ON CONFLICT(account_id) DO UPDATE SET reason=excluded.reason,blocked_at=CURRENT_TIMESTAMP,blocked_by_account_id=excluded.blocked_by_account_id,unblocked_at=NULL,unblocked_by_account_id=NULL").bind(accountId,context.user.accountId),
    auditStatement(env,context,"account.blocked","account",String(accountId),{approval_status:account.approval_status},{approval_status:"disabled",is_active:0,blocked:true})
  ]);
  return redirect(request,"/ui-v2/players/access?saved=blocked");
}
...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  if(linked&&!linked.is_owner&&context.user.canManageAccounts)accountControls.push(linked.is_active?`<form method="post" action="/ui-v2/players/${player.id}/login-disable" data-confirm="Disable this player’s login access and revoke their active sessions?" data-confirm-title="Disable Login" data-confirm-button="Disable Login"><button class="manage-button secondary" type="submit">Disable Login</button></form>`:`<form method="post" action="/ui-v2/players/${player.id}/login-enable"><button class="manage-button" type="submit">Enable Login</button></form>`);
  if(linked&&!linked.is_owner&&context.user.canManageMembership)accountControls.push(`<form method="post" action="/ui-v2/players/${player.id}/disconnect" data-confirm="Disconnect this Discord login? Their sessions and access groups will be removed, but the player record will be kept." data-confirm-title="Disconnect Discord" data-confirm-button="Disconnect"><button class="manage-button danger" type="submit">Disconnect Discord</button></form>`);
  ...
  const membershipAction=player.is_active
    ?linked?.is_owner
      ?'<p class="membership-note">This player is linked to the platform Owner and cannot be moved to Former Players. Ownership must be transferred first.</p>'
      :`<form method="post" action="/ui-v2/players/${player.id}/deactivate" data-confirm="Move this player to Former Players? A linked non-owner Discord login will be removed." data-confirm-title="Move to Former Players" data-confirm-button="Move player"><button class="manage-button danger" type="submit">Move to Former Players</button></form><p class="membership-note">A linked non-owner login is disconnected and must be approved again if the player returns.</p>`
    :`<form method="post" action="/ui-v2/players/${player.id}/reactivate"><button class="manage-button" type="submit">Return to active roster</button></form><p class="membership-note">Discord access is not restored automatically.</p>`;
  const membership=context.user.canManageMembership?`<section class="manage-card"><h2>Membership</h2><p>${player.is_active?"Moving a player to Former Players preserves their record and history.":"Return this player to the active roster when they rejoin."}</p><div class="membership-actions">${membershipAction}</div></section>`:"";
  ...
  const awayTab=awayPanel?'<button class="profile-tab" type="button" role="tab" aria-selected="false" aria-controls="away-breaks" data-profile-tab="away-breaks" tabindex="-1">Away &amp; Breaks</button>':"";
  const awayTabPanel=awayPanel?`<section class="profile-tab-panel" id="away-breaks" role="tabpanel" data-profile-panel hidden>${awayPanel}</section>`:"";
...
async function updatePlayer(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  try{
    ...
    if(value.base!==old.base_level)try{await postDiscordBaseAnnouncement(env,context,playerId,old.base_level,value.base)}catch(error){console.error("Discord base announcement failed",playerId,error)}
...
export async function handleUiV2PlayerManagement(request:Request,env:UiV2Env,context:UiV2Context):Promise<Response|null>{
  ...
  const action=path.match(/^\/ui-v2\/players\/access\/(\d+)\/(approve|reject|block)$/);
  if(request.method==="POST"&&action&&validId(action[1]))return action[2]==="approve"?approveAccount(request,env,context,Number(action[1])):action[2]==="block"?blockAccount(request,env,context,Number(action[1])):rejectAccount(request,env,context,Number(action[1]));
