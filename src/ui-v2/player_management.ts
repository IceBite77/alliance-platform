...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  const body=`${managementCss}${profileCss}<a class="manage-back" href="/ui-v2/players">← Players</a>${notice}<div class="profile-layout"><div class="profile-stack">${details}${historyCard}</div><aside class="profile-stack">${account}${membership}</aside></div>`;
