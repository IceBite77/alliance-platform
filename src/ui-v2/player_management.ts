...
import {getPlayerMaxBaseLevel,readPlayerProfileForm} from "../player_profile_fields";
...
function rankOption(rank:RankRow){
  const title=rank.display_name.trim();
  return `<option value="${rank.rank_level}">R${rank.rank_level}${title.toLowerCase()!==`r${rank.rank_level}`?` · ${esc(title)}`:""}</option>`;
}
