...
async function playerPage(request:Request,env:UiV2Env,context:UiV2Context,playerId:number){
  ...
  const overlordSummary=`<section class="loadout-card overlord-card"><h4>Overlord</h4><p class="membership-note"><strong>${currentOverlord.level?`Level ${currentOverlord.level}`:"Level not set"}</strong> · ${currentOverlord.assigned_squad?`Used with Squad ${currentOverlord.assigned_squad}`:"Squad not assigned"}</p></section>`;
