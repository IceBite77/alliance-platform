...
type TrainRow={schedule_date:string;driver_player_id:number;driver_name:string;vip_player_id:number|null;vip_name:string|null;recorded_by_name:string|null;updated_at:string};
...
const loadSchedule=async(env:UiV2Env)=>(await env.DB.prepare("SELECT t.schedule_date,t.driver_player_id,d.display_name driver_name,t.vip_player_id,v.display_name vip_name,COALESCE(NULLIF(TRIM(t.recorded_by_name),''),a.display_name) recorded_by_name,t.updated_at FROM alliance_train_schedule t JOIN players d ON d.id=t.driver_player_id LEFT JOIN players v ON v.id=t.vip_player_id LEFT JOIN accounts a ON a.id=t.updated_by_account_id WHERE t.schedule_date>=date('now','-14 days') ORDER BY t.schedule_date ASC").all<TrainRow>()).results??[];
...
async function save(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  await env.DB.batch([
    env.DB.prepare("INSERT INTO alliance_train_schedule (schedule_date,driver_player_id,vip_player_id,updated_by_account_id,recorded_by_name,updated_at) VALUES (?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(schedule_date) DO UPDATE SET driver_player_id=excluded.driver_player_id,vip_player_id=excluded.vip_player_id,updated_by_account_id=excluded.updated_by_account_id,recorded_by_name=excluded.recorded_by_name,updated_at=CURRENT_TIMESTAMP").bind(date,driver,vip,context.user.accountId,context.user.displayName),
