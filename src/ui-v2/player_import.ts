...
type ExistingPlayer={id:number;display_name:string;rank:number;base_level:number|null;player_power:number|null;is_active:number;squad1:number|null};
type ImportAction="add"|"update"|"reactivate"|"unchanged";
type ImportRow={playerId:number|null;name:string;rank:number;base:number|null;strength:number|null;squad1:number|null;action:ImportAction;errors:string[]};
...
const playerChanged=(player:ExistingPlayer,row:Pick<ImportRow,"name"|"rank"|"base"|"strength"|"squad1">)=>player.display_name.trim()!==row.name.trim()||player.rank!==row.rank||player.base_level!==row.base||player.player_power!==row.strength||player.squad1!==row.squad1;
...
.import-back{display:inline-flex;margin-bottom:18px;color:var(--ui-navigation);font-size:.78rem;font-weight:850;text-decoration:none}.import-back:hover{color:var(--ui-hover)}.import-notice{margin-bottom:17px;padding:12px 14px;border:1px solid color-mix(in srgb,var(--ui-danger) 48%,var(--ui-line));border-radius:11px;background:color-mix(in srgb,var(--ui-danger) 9%,var(--ui-surface-2));color:var(--ui-danger);font-size:.8rem;font-weight:800}.import-notice.good{border-color:color-mix(in srgb,var(--ui-success) 45%,var(--ui-line));background:color-mix(in srgb,var(--ui-success) 9%,var(--ui-surface-2));color:var(--ui-success)}.import-card{padding:21px;border:1px solid var(--ui-line);border-radius:15px;background:var(--ui-surface-2)}.import-card h2{margin:0 0 6px;font-size:1.05rem}.import-card p{margin:0;color:var(--ui-muted);font-size:.8rem;line-height:1.5}.file-field{display:block;margin-top:18px}.file-field span{display:block;margin-bottom:7px;color:var(--ui-secondary);font-size:.75rem;font-weight:850}.file-field input{width:100%;padding:11px;border:1px solid var(--ui-line-strong);border-radius:9px;background:var(--ui-surface-3);color:var(--ui-text)}.import-hint{margin-top:9px;color:var(--ui-muted);font-size:.69rem;line-height:1.5}.import-actions{display:flex;justify-content:flex-end;gap:9px;margin-top:18px}.import-button{display:inline-flex;align-items:center;justify-content:center;padding:11px 14px;border:0;border-radius:9px;background:var(--ui-button);color:var(--ui-button-text);font-size:.76rem;font-weight:900;text-decoration:none;cursor:pointer}.import-button.secondary{border:1px solid var(--ui-line-strong);background:var(--ui-surface-3);color:var(--ui-navigation)}.import-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px;margin-bottom:15px}.import-summary div{padding:12px 14px;border:1px solid var(--ui-line);border-radius:11px;background:var(--ui-surface-2)}.import-summary span,.import-summary strong{display:block}.import-summary span{color:var(--ui-muted);font-size:.64rem;font-weight:900;text-transform:uppercase;letter-spacing:.06em}.import-summary strong{margin-top:4px}.table-wrap{overflow:auto;border:1px solid var(--ui-line);border-radius:13px;background:var(--ui-surface-2)}.import-table{width:100%;min-width:880px;border-collapse:collapse}.import-table th,.import-table td{padding:10px 11px;border-bottom:1px solid var(--ui-line);font-size:.73rem;text-align:left;white-space:nowrap}.import-table th{color:var(--ui-muted);font-size:.64rem;text-transform:uppercase;letter-spacing:.05em}.import-table tr:last-child td{border-bottom:0}.row-status{font-weight:900}.row-status.add{color:var(--ui-success)}.row-status.update{color:var(--ui-secondary)}.row-status.reactivate{color:var(--ui-warning)}.row-status.unchanged{color:var(--ui-muted)}.row-error{color:var(--ui-danger);white-space:normal!important;min-width:220px}.payload-form{margin:0}@media(max-width:700px){.import-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.import-actions{display:grid;grid-template-columns:1fr}.import-button{width:100%}}
</style>`;

const loadExisting=async(env:UiV2Env)=>(await env.DB.prepare("SELECT p.id,p.display_name,p.rank,p.base_level,p.player_power,p.is_active,(SELECT power FROM player_squads s WHERE s.player_id=p.id AND s.squad_number=1 LIMIT 1) squad1 FROM players p ORDER BY p.id").all<ExistingPlayer>()).results??[];
...
async function parseSpreadsheet(env:UiV2Env,context:UiV2Context,file:File):Promise<ImportRow[]>{
  ...
  return raw.map(source=>{
    ...
    const rank=parseRank(cell(source,["rank","alliance rank","r rank"]),rankRows,matched?.rank??1),baseValue=cell(source,["base level","base","base size","hq level","headquarters level"]),base=hasValue(baseValue)?Number(baseValue):matched?.base_level??null,strengthValue=cell(source,["strength","player power","total power","power","overall power"]),strength=hasValue(strengthValue)?parsePower(strengthValue):matched?.player_power??null,squadValue=cell(source,["squad 1 power","squad1 power","squad 1 strength","squad1 strength","squad 1"]),squad1=hasValue(squadValue)?parsePower(squadValue):matched?.squad1??null;
    ...
    const next={name,rank:Number.isFinite(rank)?rank:1,base:Number.isFinite(base as number)?base:null,strength:Number.isFinite(strength as number)?strength:null,squad1:Number.isFinite(squad1 as number)?squad1:null};
    const action:ImportAction=!matched?"add":!matched.is_active?"reactivate":playerChanged(matched,next)?"update":"unchanged";
    return{playerId:matched?.id??id,name:next.name,rank:next.rank,base:next.base,strength:next.strength,squad1:next.squad1,action,errors};
  });
}

function uploadPage(context:UiV2Context,message=""){
  const body=`${importCss}<a class="import-back" href="/ui-v2/players">← Players</a>${message?`<div class="import-notice">${esc(message)}</div>`:""}<section class="import-card"><h2>Roster spreadsheet</h2><p>Upload the current roster to add new players and update existing records. You will see a full preview before anything is saved.</p><form method="post" action="/ui-v2/players/import/preview" enctype="multipart/form-data"><label class="file-field"><span>Excel or CSV file</span><input type="file" name="file" accept=".xlsx,.csv" required></label><div class="import-hint">Recognised columns: Player ID (optional), Player Name, Rank, Base Level, Strength and Squad 1 Power. Blank data cells retain existing values when a player is matched.</div><div class="import-actions"><a class="import-button secondary" href="/ui-v2/players/import/template.csv">Download template</a><button class="import-button" type="submit">Preview roster update</button></div></form></section>`;
...
function previewPage(context:UiV2Context,rows:ImportRow[]){
  ...
  const table=rows.map((row,index)=>`<tr><td>${index+1}</td><td>${row.playerId??"—"}</td><td>${esc(row.name||"—")}</td><td>R${row.rank}</td><td>${row.base??"—"}</td><td>${row.strength??"—"}</td><td>${row.squad1??"—"}</td><td><span class="row-status ${row.action}">${actionLabel(row.action)}</span></td><td class="${row.errors.length?"row-error":""}">${row.errors.length?esc(row.errors.join("; ")):"Ready"}</td></tr>`).join("");
  const payload=esc(JSON.stringify(rows.map(({action,errors,...row})=>row)));
  const body=`${importCss}<a class="import-back" href="/ui-v2/players/import">← Choose another file</a><div class="import-notice ${bad?"":"good"}">${bad?"Nothing has been saved. Correct the highlighted spreadsheet rows and upload again.":`${rows.length} rows validated and ready.`}</div><div class="import-summary"><div><span>Add</span><strong>${counts.add}</strong></div><div><span>Update</span><strong>${counts.update}</strong></div><div><span>Reactivate</span><strong>${counts.reactivate}</strong></div><div><span>No change</span><strong>${counts.unchanged}</strong></div></div><div class="table-wrap"><table class="import-table"><thead><tr><th>#</th><th>Player ID</th><th>Player</th><th>Rank</th><th>Base</th><th>Strength</th><th>Squad 1</th><th>Action</th><th>Validation</th></tr></thead><tbody>${table}</tbody></table></div><div class="import-actions"><a class="import-button secondary" href="/ui-v2/players/import">Cancel</a>${bad?"":`<form class="payload-form" method="post" action="/ui-v2/players/import/commit"><textarea name="payload" hidden>${payload}</textarea><button class="import-button" type="submit">Apply roster update</button></form>`}</div>`;
...
async function commitImport(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  try{prepared=rows.map(raw=>{const playerId=raw.playerId===null?null:Number(raw.playerId),name=String(raw.name||"").trim(),rank=Number(raw.rank),base=raw.base===null?null:Number(raw.base),strength=raw.strength===null?null:Number(raw.strength),squad1=raw.squad1===null?null:Number(raw.squad1),matched=chooseExisting(Number.isInteger(playerId)?playerId:null,name,existing),key=normalName(name),conflict=matched&&existing.some(player=>player.id!==matched.id&&player.is_active===1&&normalName(player.display_name)===key);if(!name||name.length>80||seenNames.has(key)||!Number.isInteger(rank)||rank<1||rank>5||base!==null&&(!Number.isInteger(base)||base<1||base>maxBase)||strength!==null&&(!Number.isInteger(strength)||strength<0)||squad1!==null&&(!Number.isInteger(squad1)||squad1<0)||playerId!==null&&!matched||matched&&seenIds.has(matched.id)||conflict||(rank===5||matched?.rank===5)&&rank!==matched?.rank&&!context.user.canManageProtectedRank)throw new Error(name||"Invalid row");seenNames.add(key);if(matched)seenIds.add(matched.id);return{playerId,name,rank,base,strength,squad1,matched}})}catch{return uploadPage(context,"The preview data no longer matches the roster. Upload the spreadsheet again.")}
  let added=0,updated=0,reactivated=0,unchanged=0;
  try{
    for(const row of prepared){
      const values={display_name:row.name,rank:row.rank,base_level:row.base,player_power:row.strength,squad_1_power:row.squad1};
      if(!row.matched){
        const inserted=await env.DB.prepare("INSERT INTO players (display_name,rank,base_level,player_power,is_active) VALUES (?,?,?,?,1) RETURNING id").bind(row.name,row.rank,row.base,row.strength).first<{id:number}>();if(!inserted?.id)throw new Error("Player insert failed");
        const statements=[env.DB.prepare("INSERT INTO player_rank_history (player_id,old_rank,new_rank,changed_by_account_id,note) VALUES (?,NULL,?,?,?)").bind(inserted.id,row.rank,context.user.accountId,"Player imported"),audit(env,context,"player.imported",String(inserted.id),null,values)];
      ...
      if(!changed&&player.is_active){unchanged++;continue;}
      const statements=[env.DB.prepare("UPDATE players SET display_name=?,rank=?,base_level=?,player_power=?,is_active=1,left_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(row.name,row.rank,row.base,row.strength,player.id),audit(env,context,player.is_active?"player.import.updated":"player.import.reactivated",String(player.id),{display_name:player.display_name,rank:player.rank,base_level:player.base_level,player_power:player.player_power,is_active:player.is_active,squad_1_power:player.squad1},values)];
