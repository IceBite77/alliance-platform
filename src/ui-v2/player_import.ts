...
const formatPower=(value:number|null)=>{if(value===null)return "—";const absolute=Math.abs(value),unit=absolute>=1e9?[1e9,"B"]:absolute>=1e6?[1e6,"M"]:absolute>=1e3?[1e3,"K"]:null;return unit?`${Number((value/(unit[0] as number)).toFixed(2))}${unit[1]}`:new Intl.NumberFormat("en-GB").format(value)};
...
function uploadPage(context:UiV2Context,message=""){
  const body=`${importCss}${message?`<div class="import-notice">${esc(message)}</div>`:""}<section class="import-card"><h2>Roster spreadsheet</h2><p>Upload the current roster to add new players and update existing records. Each upload also creates a dated performance snapshot for weekly history.</p><form method="post" action="/ui-v2/players/import/preview" enctype="multipart/form-data"><label class="file-field"><span>Excel or CSV file</span><input type="file" name="file" accept=".xlsx,.csv" required></label><div class="import-hint">Recognised columns: Player ID (optional), Player Name, Rank, Base Level, Total Strength, Squad 1 Power and Total Hero Power. Power can use full values or K, M and B abbreviations. Blank data cells retain existing values when a player is matched. Squads 2–4, Tactical Drone and Overlord are maintained manually on each player profile.</div><div class="import-actions"><a class="import-button secondary" href="/ui-v2/players/import/template.csv">Download template</a><button class="import-button" type="submit">Preview roster update</button></div></form></section>`;
  return uiV2Html(renderUiV2Shell(context,{eyebrow:"Players",title:"Upload Excel Roster",description:"Update the alliance roster in bulk without entering every player by hand.",body,activePath:"/ui-v2/players",back:{href:"/ui-v2/players",label:"Players"}}));
}
...
function previewPage(context:UiV2Context,rows:ImportRow[]){
  ...
  const table=rows.map((row,index)=>`<tr><td>${index+1}</td><td>${row.playerId??"—"}</td><td>${esc(row.name||"—")}</td><td>R${row.rank}</td><td>${row.base??"—"}</td><td>${formatPower(row.strength)}</td><td>${formatPower(row.squad1)}</td><td>${formatPower(row.heroPower)}</td><td><span class="row-status ${row.action}">${actionLabel(row.action)}</span></td><td class="${row.errors.length?"row-error":""}">${row.errors.length?esc(row.errors.join("; ")):"Ready"}</td></tr>`).join("");
  const payload=esc(JSON.stringify(rows.map(({action,errors,...row})=>row)));
  const body=`${importCss}<div class="import-notice ${bad?"":"good"}">${bad?"Nothing has been saved. Correct the highlighted spreadsheet rows and upload again.":`${rows.length} rows validated and ready. A weekly snapshot will be recorded for every row.`}</div><div class="import-summary"><div><span>Add</span><strong>${counts.add}</strong></div><div><span>Update</span><strong>${counts.update}</strong></div><div><span>Reactivate</span><strong>${counts.reactivate}</strong></div><div><span>No change</span><strong>${counts.unchanged}</strong></div></div><div class="table-wrap"><table class="import-table"><thead><tr><th>#</th><th>Player ID</th><th>Player</th><th>Rank</th><th>Base</th><th>Total Strength</th><th>Squad 1</th><th>Hero Power</th><th>Action</th><th>Validation</th></tr></thead><tbody>${table}</tbody></table></div><div class="import-actions"><a class="import-button secondary" href="/ui-v2/players/import">Cancel</a>${bad?"":`<form class="payload-form" method="post" action="/ui-v2/players/import/commit"><textarea name="payload" hidden>${payload}</textarea><button class="import-button" type="submit">Apply roster update</button></form>`}</div>`;
  return uiV2Html(renderUiV2Shell(context,{eyebrow:"Players",title:"Roster Preview",description:"Review every proposed change before updating player records.",body,activePath:"/ui-v2/players",back:{href:"/ui-v2/players/import",label:"Choose another file"}}));
}
...
async function commitImport(request:Request,env:UiV2Env,context:UiV2Context){
  const canScreenshotImport=await playerPermitted(env,{id:context.user.accountId,is_owner:context.user.isOwner?1:0},"screenshot_import.roster");
  if(!(context.user.canManageMembership||canScreenshotImport)||!playerSameOrigin(request,env))return new Response("Forbidden",{status:403});
