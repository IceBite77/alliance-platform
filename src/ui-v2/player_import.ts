...
function previewPage(context:UiV2Context,rows:ImportRow[]){
  ...
  const body=`${importCss}<a class="import-button secondary import-return" href="/ui-v2/players/import">← Choose another file</a><div class="import-notice ${bad?"":"good"}">${bad?"Nothing has been saved. Correct the highlighted spreadsheet rows and upload again.":`${rows.length} rows validated and ready.`}</div><div class="import-summary"><div><span>Add</span><strong>${counts.add}</strong></div><div><span>Update</span><strong>${counts.update}</strong></div><div><span>Reactivate</span><strong>${counts.reactivate}</strong></div><div><span>No change</span><strong>${counts.unchanged}</strong></div></div><div class="table-wrap"><table class="import-table"><thead><tr><th>#</th><th>Player ID</th><th>Player</th><th>Rank</th><th>Base</th><th>Strength</th><th>Squad 1</th><th>Action</th><th>Validation</th></tr></thead><tbody>${table}</tbody></table></div><div class="import-actions"><a class="import-button secondary" href="/ui-v2/players/import">Cancel</a>${bad?"":`<form class="payload-form" method="post" action="/ui-v2/players/import/commit"><textarea name="payload" hidden>${payload}</textarea><button class="import-button" type="submit">Apply roster update</button></form>`}</div>`;
