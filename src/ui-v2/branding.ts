...
function themeVars(palette:ThemePalette){
  return {
    ...
    primary:"--ui-accent",secondary:"--ui-secondary",mainText:"--ui-text",mutedText:"--ui-muted",
...
function artworkSection(source:Record<string,string>,context:UiV2Context){
  ...
  const remove=(action:string,label:string)=>`<form method="post" action="/ui-v2/branding" data-confirm="Remove this image?" data-confirm-title="Remove branding image" data-confirm-button="Remove image"><input type="hidden" name="action" value="${action}"><button class="theme-button remove-button" type="submit">${esc(label)}</button></form>`;
...
function brandingScript(){
  ...
  document.querySelectorAll('.asset-form').forEach(form=>{const input=form.querySelector('input[type=file]');if(!input)return;const icon=form.classList.contains('icon-form');form.addEventListener('submit',async event=>{clear(form);const file=input.files&&input.files[0];if(!file){event.preventDefault();show(form,'No image selected','Choose an image before uploading.');return}if(file.size>5*1024*1024){event.preventDefault();show(form,'Image is too large','Choose an image smaller than 5 MB.');return}const allowed=['image/png','image/jpeg','image/webp'];if(!allowed.includes(file.type)){event.preventDefault();show(form,'Unsupported image','Choose a PNG, JPG or WebP image.');return}if(!icon)return;event.preventDefault();let image;try{image=await createImageBitmap(file)}catch(_){show(form,'Unsupported image','Choose a valid PNG, JPG or WebP image.');return}const data=new FormData();data.append('action','save_icons');const make=size=>new Promise((resolve,reject)=>{const canvas=document.createElement('canvas');canvas.width=canvas.height=size;const drawing=canvas.getContext('2d');if(!drawing){reject(new Error('Canvas is unavailable'));return}drawing.clearRect(0,0,size,size);const scale=Math.min(size/image.width,size/image.height),width=image.width*scale,height=image.height*scale;drawing.drawImage(image,(size-width)/2,(size-height)/2,width,height);canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('Icon creation failed')),'image/png')});try{for(const [field,size] of [['master',512],['i16',16],['i32',32],['i48',48],['apple',180],['pwa192',192],['pwa512',512]])data.append(field,await make(size),field+'.png')}catch(_){image.close();show(form,'Icon creation failed','This browser could not prepare the icon sizes. Try another image or browser.');return}image.close();const button=form.querySelector('button'),original=button?.textContent||'Upload & create icon set';if(button){button.disabled=true;button.textContent='Creating icon set…'}try{const response=await fetch('/ui-v2/branding',{method:'POST',body:data});if(!response.ok)throw new Error('Upload failed');location.href=response.url||'/ui-v2/branding'}catch(_){if(button){button.disabled=false;button.textContent=original}show(form,'Upload failed','The icon set could not be saved. Please try again.')}});});})();</script>`;
}
...
function page(context:UiV2Context,source:Record<string,string>,message:string|null){
  ...
  return uiV2Html(renderUiV2Shell(context,{eyebrow:"Settings · Branding",title:"Branding",description:"Choose a coordinated theme, fine-tune individual colours, and save reusable palettes without redesigning each page.",body,activePath:"/ui-v2/branding",back:{href:"/ui-v2/settings",label:"Alliance Settings"}}));
}
...
async function handlePost(request:Request,env:UiV2Env,context:UiV2Context,source:Record<string,string>){
  ...
  if(action==="save_icons"){
    ...
    try{
      ...
      await audit(env,context,"branding.icons.generated",{source:"single artwork upload",sizes:[16,32,48,180,192,512]});
    }catch(error){
      console.error("UI v2 icon upload failed",error);
      return Response.redirect(new URL("/ui-v2/branding?error=icons",request.url),303);
    }
...
export async function handleUiV2Branding(request:Request,env:UiV2Env):Promise<Response|null>{
  ...
  const message=error==="image"?"That image could not be used. Check its format and size, then try again.":error==="icons"?"The icon sizes were created, but the platform could not save them. Please try again.":messages[url.searchParams.get("saved")||""]||null;
