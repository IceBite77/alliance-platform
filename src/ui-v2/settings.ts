...
import {discordTabs} from "./discord_tabs";
...
async function discordPage(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  const tabs=current?discordTabs("overview"):"";
  ...
  const body=`${css}${message}${tabs}<section class="settings-panel">${status}${actions}</section>`;
  return uiV2Html(renderUiV2Shell(context,{eyebrow:"Settings",title:"Discord",description:"Connect and manage the alliance Discord server used for sign-in and platform notifications.",body,activePath:"/ui-v2/settings/discord",back:{href:"/ui-v2/settings",label:"Alliance Settings"}}));
}
