...
const botInstallUrl=(env:UiV2Env)=>{if(!env.DISCORD_CLIENT_ID)return null;const url=new URL("https://discord.com/oauth2/authorize");url.searchParams.set("client_id",env.DISCORD_CLIENT_ID);url.searchParams.set("scope","bot applications.commands");url.searchParams.set("permissions","84992");return url.toString()};
...
async function discordPage(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  let actions=current?`<div class="settings-actions"><a class="settings-button" href="/ui-v2/settings/discord-notifications">Choose notification channels</a><a class="settings-button secondary" href="/ui-v2/settings/discord?action=change">Change server</a><a class="settings-button danger" href="/ui-v2/settings/discord?action=disconnect">Disconnect server</a></div>`:`<div class="settings-actions">${connect}</div>`;
