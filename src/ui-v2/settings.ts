...
const back='<a class="settings-button secondary return-settings" href="/ui-v2/settings">← Settings</a>';
const forbidden=(context:UiV2Context,title:string)=>uiV2Html(renderUiV2Shell(context,{eyebrow:"Settings",title,description:"You do not have permission to manage this section.",body:"",activePath:"/ui-v2/settings"}),403);
...
async function home(env:UiV2Env,context:UiV2Context){
  ...
  const [guild,settingRows]=await Promise.all([env.DB.prepare("SELECT guild_name FROM discord_guild_connection WHERE id=1").first<{guild_name:string}>(),env.DB.prepare("SELECT key,value FROM settings WHERE key IN ('alliance_timezone','player_max_base_level','player_max_overlord_level')").all<SettingRow>()]);
  ...
  return uiV2Html(renderUiV2Shell(context,{eyebrow:"Configuration",title:"Alliance Settings",description:"Everything that defines how this alliance looks, connects and labels itself lives here.",body:`${css}<div class="settings-grid">${cards}</div>`,activePath:"/ui-v2/settings"}));
}
...
async function savePlayerSettings(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  const form=await request.formData(),base=Number(form.get("max_base_level")),overlord=Number(form.get("max_overlord_level"));
  if(!Number.isInteger(base)||base<1||base>999||!Number.isInteger(overlord)||overlord<1||overlord>999)return redirect(request,"/ui-v2/settings/players?error=Please%20enter%20valid%20level%20limits.");
  const old=settings(await env.DB.prepare("SELECT key,value FROM settings WHERE key IN ('player_max_base_level','player_max_overlord_level')").all<SettingRow>()),next={player_max_base_level:base,player_max_overlord_level:overlord};
  await env.DB.batch([env.DB.prepare("INSERT INTO settings (key,value,updated_at) VALUES ('player_max_base_level',?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP").bind(String(base)),env.DB.prepare("INSERT INTO settings (key,value,updated_at) VALUES ('player_max_overlord_level',?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP").bind(String(overlord)),audit(env,context,"settings.players.updated","settings","player_limits",old,next)]);
  return redirect(request,"/ui-v2/settings/players?saved=1");
}

const botInstallUrl=(env:UiV2Env)=>{if(!env.DISCORD_CLIENT_ID)return null;const url=new URL("https://discord.com/oauth2/authorize");url.searchParams.set("client_id",env.DISCORD_CLIENT_ID);url.searchParams.set("scope","bot applications.commands");url.searchParams.set("permissions","3072");return url.toString()};
...
async function discordPage(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  let actions=current?`<div class="settings-actions"><a class="settings-button secondary" href="/ui-v2/settings/discord?action=change">Change server</a><a class="settings-button danger" href="/ui-v2/settings/discord?action=disconnect">Disconnect server</a></div>`:`<div class="settings-actions">${connect}</div>`;
  ...
  const body=`${css}${back}${message}<section class="settings-panel">${status}${actions}</section>`;
  return uiV2Html(renderUiV2Shell(context,{eyebrow:"Settings",title:current?"Discord Connected":"Connect Discord",description:"Connect the alliance Discord server used for sign-in and platform notifications.",body,activePath:"/ui-v2/settings/discord"}));
}
...
async function verifyDiscord(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  try{
    ...
    return redirect(request,"/ui-v2/settings/discord?connected=1");
