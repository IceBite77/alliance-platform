...
function playerRows(players:PlayerRow[]){
  return players.map(player=>{
    ...
    const linked=player.account_id&&player.account_status==="active";
    return `<a class="player-row" href="/players/${player.id}" data-player-row data-state="${player.is_active?"active":"former"}" data-search="${esc(search)}"><div class="rank-mark">${rankArt}</div><div><div class="player-name">${esc(player.display_name)}</div><div class="player-meta">${esc(meta)}</div></div><div class="player-state"><span class="state-label ${player.is_active?"active":"former"}">${player.is_active?"Active member":"Former member"}</span><span class="login-state${linked?" linked":""}">${linked?"Discord linked":"No active login"}</span></div><span class="row-arrow">›</span></a>`;
  }).join("");
}

const playersScript=`<script>(()=>{const search=document.querySelector('[data-player-search]'),rows=[...document.querySelectorAll('[data-player-row]')],buttons=[...document.querySelectorAll('[data-player-filter]')],count=document.querySelector('[data-player-count]'),empty=document.querySelector('[data-player-empty]');let filter='active';const update=()=>{const term=(search?.value||'').trim().toLowerCase();let shown=0;rows.forEach(row=>{const visible=(filter==='all'||row.dataset.state===filter)&&(!term||row.dataset.search.includes(term));row.hidden=!visible;if(visible)shown++});if(count)count.textContent=shown+' '+(shown===1?'player':'players');empty?.classList.toggle('show',shown===0)};search?.addEventListener('input',update);buttons.forEach(button=>button.addEventListener('click',()=>{filter=button.dataset.playerFilter;buttons.forEach(item=>{const active=item===button;item.classList.toggle('active',active);item.setAttribute('aria-pressed',String(active))});update()}));update();})();</script>`;

export async function renderUiV2Players(env:UiV2Env,context:UiV2Context){
  ...
  const [playerResult,pendingResult]=await Promise.all([
    env.DB.prepare(`SELECT p.id,p.display_name,p.rank,p.base_level,p.is_active,p.joined_at,p.left_at,r.display_name rank_name,r.image_path rank_image,a.id account_id,a.approval_status account_status,i.provider_username discord_username FROM players p LEFT JOIN alliance_ranks r ON r.rank_level=p.rank LEFT JOIN accounts a ON a.player_id=p.id LEFT JOIN account_identities i ON i.account_id=a.id AND i.provider='discord' ORDER BY p.is_active DESC,p.display_name COLLATE NOCASE`).all<PlayerRow>(),
  ...
  const active=players.filter(player=>player.is_active).length,former=players.length-active,linked=players.filter(player=>player.account_id&&player.account_status==="active").length;
  const waiting=pending.length?`<div class="waiting-panel"><div><strong>${pending.length} ${pending.length===1?"account is":"accounts are"} waiting for access</strong><span>${esc(pending.slice(0,3).map(item=>item.provider_username||item.display_name).join(", "))}${pending.length>3?` and ${pending.length-3} more`:""}</span></div><a href="/leadership/players">Review access</a></div>`:"";
  const body=`${playersCss}<div class="player-summary"><div class="summary-item"><span>Active</span><strong>${active}</strong></div><div class="summary-item"><span>Former</span><strong>${former}</strong></div><div class="summary-item"><span>Discord linked</span><strong>${linked}</strong></div><div class="summary-item"><span>Waiting access</span><strong>${pending.length}</strong></div></div>${waiting}<div class="players-toolbar"><input class="player-search" type="search" placeholder="Search players, ranks or Discord…" aria-label="Search players" data-player-search><div class="filter-set" aria-label="Player status filter"><button class="filter-button active" type="button" aria-pressed="true" data-player-filter="active">Active</button><button class="filter-button" type="button" aria-pressed="false" data-player-filter="former">Former</button><button class="filter-button" type="button" aria-pressed="false" data-player-filter="all">All</button></div><a class="add-player" href="/players/new">+ Add player</a></div><p class="result-line"><span data-player-count>${active} players</span> shown</p><div class="player-list">${playerRows(players)}</div><div class="empty-state" data-player-empty>No players match this view.</div>${playersScript}`;
