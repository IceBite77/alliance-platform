...
async function page(request:Request,env:UiV2Env,context:UiV2Context){
  ...
  const body=`${css}${message(new URL(request.url))}<div class="key-tabs" role="tablist" aria-label="Integration and key sections"><button class="key-tab active" type="button" role="tab" aria-selected="true" aria-controls="licensing" data-key-tab="licensing">ICE Licensing</button><button class="key-tab" type="button" role="tab" aria-selected="false" aria-controls="openai" data-key-tab="openai" tabindex="-1">OpenAI</button><button class="key-tab" type="button" role="tab" aria-selected="false" aria-controls="discord-bot" data-key-tab="discord-bot" tabindex="-1">Discord Bot</button><button class="key-tab" type="button" role="tab" aria-selected="false" aria-controls="discord-app" data-key-tab="discord-app" tabindex="-1">Discord Application</button><button class="key-tab" type="button" role="tab" aria-selected="false" aria-controls="security" data-key-tab="security" tabindex="-1">Security</button></div>
  <section class="key-panel key-card" id="licensing" role="tabpanel" data-key-panel><h2>ICE Licensing</h2><p>Replace the installation key issued by ICE Licensing. It is verified remotely and is never retained by this platform.</p><div class="key-status"><span>${esc(activation?.installation_id||"Installation unavailable")}</span><strong class="${activation?.status==="active"?"":"bad"}">${esc(activation?.status||"unknown")}</strong></div><form class="key-form" method="post" action="/ui-v2/settings/integrations/license"><label>New ICE installation key</label><input class="key-input" type="password" name="value" placeholder="ICE-XXXX-XXXX-XXXX" autocomplete="new-password" required><button class="key-button" type="submit">Verify and replace licence</button></form></section>
  <section class="key-panel key-card" id="openai" role="tabpanel" data-key-panel hidden><h2>OpenAI</h2><p>Used only for screenshot reading. A replacement is tested with OpenAI before it is encrypted and saved.</p>${status("OPENAI_API_KEY",managed,env.OPENAI_API_KEY)}<form class="key-form" method="post" action="/ui-v2/settings/integrations/secret"><input type="hidden" name="key" value="OPENAI_API_KEY"><label>New OpenAI API key</label><input class="key-input" type="password" name="value" autocomplete="new-password" required><button class="key-button" type="submit">Test and replace key</button></form></section>
  <section class="key-panel key-card" id="discord-bot" role="tabpanel" data-key-panel hidden><h2>Discord bot</h2><p>The bot token is tested against Discord before being saved. Existing connected-server details are not changed.</p>${status("DISCORD_BOT_TOKEN",managed,env.DISCORD_BOT_TOKEN)}<form class="key-form" method="post" action="/ui-v2/settings/integrations/secret"><input type="hidden" name="key" value="DISCORD_BOT_TOKEN"><label>New Discord bot token</label><input class="key-input" type="password" name="value" autocomplete="new-password" required><button class="key-button" type="submit">Test and replace token</button></form></section>
  <section class="key-panel key-card" id="discord-app" role="tabpanel" data-key-panel hidden><h2>Discord application</h2><p>Replace the application Client ID or OAuth client secret used for Discord sign-in.</p>${status("DISCORD_CLIENT_SECRET",managed,(env as any).DISCORD_CLIENT_SECRET)}<form class="key-form" method="post" action="/ui-v2/settings/integrations/secret"><input type="hidden" name="key" value="DISCORD_CLIENT_ID"><label>New Client ID</label><input class="key-input" name="value" inputmode="numeric" autocomplete="off" required><button class="key-button" type="submit">Replace Client ID</button></form><form class="key-form" method="post" action="/ui-v2/settings/integrations/secret"><input type="hidden" name="key" value="DISCORD_CLIENT_SECRET"><label>New client secret</label><input class="key-input" type="password" name="value" autocomplete="new-password" required><button class="key-button" type="submit">Replace client secret</button></form></section>
  <section class="key-panel key-card" id="security" role="tabpanel" data-key-panel hidden><h2>Emergency setup key</h2><p>Rotate the private bootstrap/recovery key. It does not alter the current owner or sign anyone out.</p>${status("SETUP_KEY",managed,(env as any).SETUP_KEY)}<form class="key-form" method="post" action="/ui-v2/settings/integrations/secret"><input type="hidden" name="key" value="SETUP_KEY"><label>New setup key</label><input class="key-input" type="password" name="value" minlength="24" autocomplete="new-password" required><button class="key-button" type="submit">Rotate setup key</button></form></section>
  <div class="key-warning">Keys are never shown after saving. Audit entries record who changed a credential and when, but never contain the credential itself.</div><script>(()=>{const tabs=[...document.querySelectorAll('[data-key-tab]')],panels=[...document.querySelectorAll('[data-key-panel]')],ids=new Set(tabs.map(tab=>tab.dataset.keyTab));const activate=(requested,focus=false)=>{const id=ids.has(requested)?requested:'licensing';tabs.forEach(tab=>{const active=tab.dataset.keyTab===id;tab.classList.toggle('active',active);tab.setAttribute('aria-selected',String(active));tab.tabIndex=active?0:-1;if(active&&focus)tab.focus()});panels.forEach(panel=>panel.hidden=panel.id!==id)};tabs.forEach((tab,index)=>{tab.addEventListener('click',()=>{history.replaceState(null,'','#'+tab.dataset.keyTab);activate(tab.dataset.keyTab)});tab.addEventListener('keydown',event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const next=event.key==='Home'?0:event.key==='End'?tabs.length-1:event.key==='ArrowRight'?(index+1)%tabs.length:(index-1+tabs.length)%tabs.length;history.replaceState(null,'','#'+tabs[next].dataset.keyTab);activate(tabs[next].dataset.keyTab,true)})});window.addEventListener('hashchange',()=>activate(location.hash.slice(1)));activate(location.hash.slice(1))})();</script>`;
