...
const polishSecurity=async(res:Response)=>{
  ...
  // Security is generated by the outer controller, so it misses the shared navigation
  // decorator used by the other admin pages. Match that familiar child-page treatment.
  html=html.replace(/<div class="head">\s*<div>\s*<h1>Security &amp; Access<\/h1>[\s\S]*?<\/div>\s*<a href="\/">← Leadership Console<\/a>\s*<\/div>/,`<div class="securitynav"><div class="securityidentity">Security &amp; Access</div><a href="/">← Leadership Console</a></div><div class="securityintro"><h1>Security &amp; Access</h1><p class="sub">Define who can access platform features. Player accounts and day-to-day approvals are managed from Players.</p></div>`);
  html=html.replace('</head>',`<style>.securitynav{display:flex;flex-direction:column;align-items:flex-end;gap:5px;padding-bottom:18px;border-bottom:1px solid #2b3850}.securitynav .securityidentity{color:#90a0bb;font-size:.76rem;font-weight:700}.securitynav a{color:#aebddd;text-decoration:none;font-weight:700}.securityintro{padding-top:24px}.securityintro h1{margin:0 0 8px}@media(max-width:650px){.securitynav{align-items:flex-start}.securityintro{padding-top:20px}}</style></head>`);
  return withHtml(res,html);
};
